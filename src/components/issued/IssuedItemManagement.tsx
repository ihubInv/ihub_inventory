        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="text-xl font-semibold text-gray-900">Item Details & Tracking</h3>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              {/* Audit Trail */}
              <div>
                <h4 className="mb-3 text-lg font-medium text-gray-900">Audit Trail</h4>
                <div className="space-y-3">
                  {getItemAuditTrail(viewingItem.id).map((entry: any, index: number) => (
                    <div key={index} className="p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-gray-500">
                          {new Date(entry.returnDate || entry.issuedDate).toLocaleString()}
                        </span>
                      </div>
                      <p className="mt-1 text-sm text-gray-600">{entry.notes}</p>
                    </div>
                  ))}
                  {getItemAuditTrail(viewingItem.id).length === 0 && (
                    <p className="text-sm text-gray-500">No audit trail available for this item.</p>
                  )}
                </div>
              </div>
            </div>
